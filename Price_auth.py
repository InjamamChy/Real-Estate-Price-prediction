...
def save_expense(cursor, db):
    ...
    with st.form(key='expense_submit_form', clear_on_submit=False, border=True):
        expense_category = ['beside road', 'PlayFields', 'GreenFields', 
                            'Others']
    ...
    edit_data(cursor, db, df, columns, 'Edit Prices', 'expense')
    delete_data(cursor, db, df, columns, 'Delete Prices', 'expense')
